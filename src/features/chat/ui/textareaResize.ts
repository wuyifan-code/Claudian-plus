...
export function autoResizeTextarea(textarea: HTMLTextAreaElement): void {
  const viewHeight = textarea.closest('.claudian-plus-container')?.clientHeight ?? window.innerHeight;
  const maxHeight = calculateTextareaMaxHeight(viewHeight);

  textarea.setCssProps({
    '--claudian-plus-textarea-min-height': `${TEXTAREA_BASE_MIN_HEIGHT}px`,
    '--claudian-plus-textarea-max-height': `${maxHeight}px`,
  });
  ...
  textarea.setCssProps({
    '--claudian-plus-textarea-min-height': `${minHeight}px`,
    '--claudian-plus-textarea-max-height': `${maxHeight}px`,
  });
}
