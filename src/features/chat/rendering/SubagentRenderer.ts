...
function createSection(parentEl: HTMLElement, title: string, bodyClass?: string): SubagentSection {
  const wrapperEl = parentEl.createDiv({ cls: 'claudian-plus-subagent-section' });

  const headerEl = wrapperEl.createDiv({ cls: 'claudian-plus-subagent-section-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  const titleEl = headerEl.createDiv({ cls: 'claudian-plus-subagent-section-title' });
  titleEl.setText(title);

  const bodyEl = wrapperEl.createDiv({ cls: 'claudian-plus-subagent-section-body' });
...
function setPromptText(promptBodyEl: HTMLElement, prompt: string): void {
  promptBodyEl.empty();
  const textEl = promptBodyEl.createDiv({ cls: 'claudian-plus-subagent-prompt-text' });
...
function renderSubagentToolContent(contentEl: HTMLElement, toolCall: ToolCallInfo): void {
  ...
  if (!toolCall.result && toolCall.status === 'running') {
    const emptyEl = contentEl.createDiv({ cls: 'claudian-plus-subagent-tool-empty' });
...
function setSubagentToolStatus(view: SubagentToolView, status: ToolCallInfo['status']): void {
  view.statusEl.className = 'claudian-plus-subagent-tool-status';
...
function updateSubagentToolView(view: SubagentToolView, toolCall: ToolCallInfo): void {
  view.wrapperEl.className = `claudian-plus-subagent-tool-item claudian-plus-subagent-tool-${toolCall.status}`;
...
function createSubagentToolView(parentEl: HTMLElement, toolCall: ToolCallInfo): SubagentToolView {
  const wrapperEl = parentEl.createDiv({
    cls: `claudian-plus-subagent-tool-item claudian-plus-subagent-tool-${toolCall.status}`,
  });
  wrapperEl.dataset.toolId = toolCall.id;

  const headerEl = wrapperEl.createDiv({ cls: 'claudian-plus-subagent-tool-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  const iconEl = headerEl.createDiv({ cls: 'claudian-plus-subagent-tool-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setToolIcon(iconEl, toolCall.name);

  const nameEl = headerEl.createDiv({ cls: 'claudian-plus-subagent-tool-name' });
  const summaryEl = headerEl.createDiv({ cls: 'claudian-plus-subagent-tool-summary' });
  const statusEl = headerEl.createDiv({ cls: 'claudian-plus-subagent-tool-status' });

  const contentEl = wrapperEl.createDiv({ cls: 'claudian-plus-subagent-tool-content' });
...
function ensureResultSection(state: SubagentState): SubagentSection {
  ...
  const section = createSection(state.contentEl, 'Result', 'claudian-plus-subagent-result-body');
  section.wrapperEl.addClass('claudian-plus-subagent-section-result');
...
function setResultText(state: SubagentState, text: string): void {
  ...
  const resultEl = section.bodyEl.createDiv({ cls: 'claudian-plus-subagent-result-output' });
...
function hydrateSyncSubagentStateFromStored(state: SubagentState, subagent: SubagentInfo): void {
  ...
  if (subagent.status === 'completed' || subagent.status === 'error') {
    ...
    state.statusEl.className = 'claudian-plus-subagent-status status-running';
...
export function createSubagentBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'claudian-plus-subagent-list' });
  wrapperEl.dataset.subagentId = taskToolId;

  const headerEl = wrapperEl.createDiv({ cls: 'claudian-plus-subagent-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  const iconEl = headerEl.createDiv({ cls: 'claudian-plus-subagent-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setIcon(iconEl, getToolIcon(TOOL_TASK));

  const labelEl = headerEl.createDiv({ cls: 'claudian-plus-subagent-label' });
  labelEl.setText(truncateDescription(description));

  const statusEl = headerEl.createDiv({ cls: 'claudian-plus-subagent-status status-running' });
  statusEl.setAttribute('aria-label', 'Status: running');

  const contentEl = wrapperEl.createDiv({ cls: 'claudian-plus-subagent-content' });

  const promptSection = createSection(contentEl, 'Prompt', 'claudian-plus-subagent-prompt-body');
  promptSection.wrapperEl.addClass('claudian-plus-subagent-section-prompt');
  setPromptText(promptSection.bodyEl, prompt);

  const toolsContainerEl = contentEl.createDiv({ cls: 'claudian-plus-subagent-tools' });
...
export function finalizeSubagentBlock(
  ...
  state.statusEl.className = 'claudian-plus-subagent-status';
...
function renderAsyncContentLikeSync(
  ...
  const resultSection = createSection(contentEl, 'Result', 'claudian-plus-subagent-result-body');
  resultSection.wrapperEl.addClass('claudian-plus-subagent-section-result');
  const resultEl = resultSection.bodyEl.createDiv({ cls: 'claudian-plus-subagent-result-output' });
...
export function createAsyncSubagentBlock(
  ...
  const statusTextEl = headerEl.createDiv({ cls: 'claudian-plus-subagent-status-text' });
...
export function markAsyncSubagentOrphaned(state: AsyncSubagentState): void {
  ...
  state.statusEl.className = 'claudian-plus-subagent-status status-error';
  state.statusEl.empty();
...
export function renderStoredAsyncSubagent(
  ...
  const statusEl = headerEl.createDiv({ cls: `claudian-plus-subagent-status ${statusIconClass}` });
