...
export function renderTodoItems(
  ...
  for (const todo of todos) {
    const item = container.createDiv({ cls: `claudian-plus-todo-item claudian-plus-todo-${todo.status}` });

    const icon = item.createSpan({ cls: 'claudian-plus-todo-status-icon' });
    icon.setAttribute('aria-hidden', 'true');
    setIcon(icon, getTodoStatusIcon(todo.status));

    const text = item.createSpan({ cls: 'claudian-plus-todo-text' });
