...
export function createWriteEditBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'claudian-plus-write-edit-block' });
  wrapperEl.dataset.toolId = toolCall.id;

  // Header (clickable to collapse/expand)
  const headerEl = wrapperEl.createDiv({ cls: 'claudian-plus-write-edit-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  // File icon
  const iconEl = headerEl.createDiv({ cls: 'claudian-plus-write-edit-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setIcon(iconEl, getToolIcon(toolName));

  const nameEl = headerEl.createDiv({ cls: 'claudian-plus-write-edit-name' });
  nameEl.setText(toolName);
  const summaryEl = headerEl.createDiv({ cls: 'claudian-plus-write-edit-summary' });
  summaryEl.setText(fileNameOnly(filePath) || 'file');

  // Populated when diff is computed
  const statsEl = headerEl.createDiv({ cls: 'claudian-plus-write-edit-stats' });

  const statusEl = headerEl.createDiv({ cls: 'claudian-plus-write-edit-status status-running' });
  statusEl.setAttribute('aria-label', 'Status: running');

  // Content area (collapsed by default)
  const contentEl = wrapperEl.createDiv({ cls: 'claudian-plus-write-edit-content' });

  // Initial loading state
  const loadingRow = contentEl.createDiv({ cls: 'claudian-plus-write-edit-diff-row' });
  const loadingEl = loadingRow.createDiv({ cls: 'claudian-plus-write-edit-loading' });
...
export function updateWriteEditWithDiff(state: WriteEditState, diffData: ToolDiffData): void {
  ...
  // Render diff content
  const row = state.contentEl.createDiv({ cls: 'claudian-plus-write-edit-diff-row' });
  const diffEl = row.createDiv({ cls: 'claudian-plus-write-edit-diff' });
...
export function finalizeWriteEditBlock(state: WriteEditState, isError: boolean): void {
  ...
  state.statusEl.className = 'claudian-plus-write-edit-status';
  ...
  if (isError) {
    ...
    if (!state.diffLines) {
      state.contentEl.empty();
      const row = state.contentEl.createDiv({ cls: 'claudian-plus-write-edit-diff-row' });
      const errorEl = row.createDiv({ cls: 'claudian-plus-write-edit-error' });
    ...
    state.contentEl.empty();
    const row = state.contentEl.createDiv({ cls: 'claudian-plus-write-edit-diff-row' });
    const doneEl = row.createDiv({ cls: 'claudian-plus-write-edit-done-text' });
    doneEl.setText('DONE');
  }
...
export function renderStoredWriteEdit(
  ...
  const statusEl = headerEl.createDiv({ cls: 'claudian-plus-write-edit-status' });
  if (isError) {
  ...
  const row = contentEl.createDiv({ cls: 'claudian-plus-write-edit-diff-row' });
