...
export class InlineAskUserQuestion {
  ...
  render(): void {
    this.rootEl = this.containerEl.createDiv({ cls: 'claudian-plus-ask-question-inline' });

    const titleEl = this.rootEl.createDiv({ cls: 'claudian-plus-ask-inline-title' });
    ...
    if (!this.config.immediateSelect) {
      this.tabBar = this.rootEl.createDiv({ cls: 'claudian-plus-ask-tab-bar' });
      this.renderTabBar();
    }
    this.contentArea = this.rootEl.createDiv({ cls: 'claudian-plus-ask-content' });
  ...
  private renderTabBar(): void {
    ...
    for (let idx = 0; idx < this.questions.length; idx++) {
      ...
      const tab = this.tabBar.createSpan({ cls: 'claudian-plus-ask-tab' });
      tab.createSpan({ text: this.questions[idx].header, cls: 'claudian-plus-ask-tab-label' });
      tab.createSpan({ text: answered ? ' \u2713' : '', cls: 'claudian-plus-ask-tab-tick' });
    ...
    const submitTab = this.tabBar.createSpan({ cls: 'claudian-plus-ask-tab' });
    submitTab.createSpan({ text: allAnswered ? '\u2713 ' : '', cls: 'claudian-plus-ask-tab-submit-check' });
    submitTab.createSpan({ text: 'Submit', cls: 'claudian-plus-ask-tab-label' });
  ...
  private renderQuestionTab(idx: number): void {
    ...
    this.contentArea.createDiv({
      text: q.question,
      cls: 'claudian-plus-ask-question-text',
    });

    const listEl = this.contentArea.createDiv({ cls: 'claudian-plus-ask-list' });
    ...
    for (let optIdx = 0; optIdx < q.options.length; optIdx++) {
      ...
      const row = listEl.createDiv({ cls: 'claudian-plus-ask-item' });
      if (isFocused) row.addClass('is-focused');
      if (isSelected) row.addClass('is-selected');

      row.createSpan({ text: isFocused ? '\u203A' : '\u00A0', cls: 'claudian-plus-ask-cursor' });
      row.createSpan({ text: `${optIdx + 1}. `, cls: 'claudian-plus-ask-item-num' });

      if (isMulti) {
        this.renderMultiSelectCheckbox(row, isSelected);
      }

      const labelBlock = row.createDiv({ cls: 'claudian-plus-ask-item-content' });
      const labelRow = labelBlock.createDiv({ cls: 'claudian-plus-ask-label-row' });
      labelRow.createSpan({ text: option.label, cls: 'claudian-plus-ask-item-label' });

      if (!isMulti && isSelected) {
        labelRow.createSpan({ text: ' \u2713', cls: 'claudian-plus-ask-check-mark' });
      }

      if (option.description) {
        labelBlock.createDiv({ text: option.description, cls: 'claudian-plus-ask-item-desc' });
      }
    ...
    if (this.canShowCustomInputForQuestion(q)) {
      ...
      const customRow = listEl.createDiv({ cls: 'claudian-plus-ask-item claudian-plus-ask-custom-item' });
      if (customFocused) customRow.addClass('is-focused');

      customRow.createSpan({ text: customFocused ? '\u203A' : '\u00A0', cls: 'claudian-plus-ask-cursor' });
      customRow.createSpan({ text: `${customIdx + 1}. `, cls: 'claudian-plus-ask-item-num' });
  ...
  private renderSubmitTab(): void {
    this.contentArea.createDiv({
      ...
      cls: 'claudian-plus-ask-review-title',
    });

    const reviewEl = this.contentArea.createDiv({ cls: 'claudian-plus-ask-review' });
    ...
    for (let idx = 0; idx < this.questions.length; idx++) {
      ...
      pairEl.createDiv({ text: `${idx + 1}.`, cls: 'claudian-plus-ask-review-num' });
      ...
      bodyEl.createDiv({
        ...
        cls: answerText ? 'claudian-plus-ask-review-a-text' : 'claudian-plus-ask-review-empty',
      });
    ...
    const actionsEl = this.contentArea.createDiv({ cls: 'claudian-plus-ask-list' });
    const allAnswered = this.questions.every((_, i) => this.isQuestionAnswered(i));

    const submitRow = actionsEl.createDiv({ cls: 'claudian-plus-ask-item' });
    if (this.focusedItemIndex === 0) submitRow.addClass('is-focused');
    if (!allAnswered) submitRow.addClass('is-disabled');
    submitRow.createSpan({ text: this.focusedItemIndex === 0 ? '\u203A' : '\u00A0', cls: 'claudian-plus-ask-cursor' });
    submitRow.createSpan({ text: '1. ', cls: 'claudian-plus-ask-item-num' });
    submitRow.createSpan({ text: 'Submit answers', cls: 'claudian-plus-ask-item-label' });
    ...
    cancelRow.createSpan({ text: this.focusedItemIndex === 1 ? '\u203A' : '\u00A0', cls: 'claudian-plus-ask-cursor' });
    cancelRow.createSpan({ text: '2. ', cls: 'claudian-plus-ask-item-num' });
  ...
  private renderMultiSelectCheckbox(parent: HTMLElement, checked: boolean): void {
    parent.createSpan({
      text: checked ? '[\u2713] ' : '[ ] ',
      cls: `claudian-plus-ask-check${checked ? ' is-checked' : ''}`,
    });
  }
  ...
  private updateOptionVisuals(qIdx: number): void {
    ...
    for (let i = 0; i < q.options.length; i++) {
      ...
      if (isMulti) {
        const checkSpan = item.querySelector('.claudian-plus-ask-check');
        if (checkSpan) {
          checkSpan.textContent = isSelected ? '[\u2713] ' : '[ ] ';
          checkSpan.toggleClass('is-checked', isSelected);
        }
      } else {
        const labelRow = item.querySelector('.claudian-plus-ask-label-row');
        const existingMark = item.querySelector('.claudian-plus-ask-check-mark');
  ...
  private updateTabIndicators(): void {
    for (let idx = 0; idx < this.questions.length; idx++) {
      ...
      const tick = tab.querySelector('.claudian-plus-ask-tab-tick');
      const answered = this.isQuestionAnswered(idx);
    ...
    if (submitTab) {
      const submitCheck = submitTab.querySelector('.claudian-plus-ask-tab-submit-check');
      const allAnswered = this.questions.every((_, i) => this.isQuestionAnswered(i));
    ...
    switch (e.key) {
      ...
      case 'Enter':
        ...
        if (this.focusedItemIndex < q.options.length) {
          ...
          const input = customRow?.querySelector('.claudian-plus-ask-custom-text') as HTMLInputElement;
          input?.focus();
        }
        break;
    }
