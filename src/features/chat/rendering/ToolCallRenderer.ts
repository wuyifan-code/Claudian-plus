...
function appendToolLink(parent: HTMLElement, title: string, url: string): void {
  const linkEl = parent.createEl('a', { cls: 'claudian-plus-tool-link' });
  linkEl.setAttribute('href', url);
  linkEl.setAttribute('target', '_blank');
  linkEl.setAttribute('rel', 'noopener noreferrer');

  const iconEl = linkEl.createSpan({ cls: 'claudian-plus-tool-link-icon' });
  setIcon(iconEl, 'external-link');

  linkEl.createSpan({ cls: 'claudian-plus-tool-link-title', text: title });
}
...
function renderWebSearchActionExpanded(container: HTMLElement, input: Record<string, unknown>): boolean {
  ...
  const linesEl = container.createDiv({ cls: 'claudian-plus-tool-lines' });

  switch (data.actionType) {
    case 'open_page':
      linesEl.createDiv({ cls: 'claudian-plus-tool-line', text: 'Open page' });
      if (data.url) {
        appendToolLink(linesEl, data.url, data.url);
      } else {
        linesEl.createDiv({ cls: 'claudian-plus-tool-line', text: 'URL unavailable' });
      }
      return true;

    case 'find_in_page':
      linesEl.createDiv({ cls: 'claudian-plus-tool-line', text: 'Find in page' });
      if (data.url) {
        appendToolLink(linesEl, data.url, data.url);
      } else {
        linesEl.createDiv({ cls: 'claudian-plus-tool-line', text: 'URL unavailable' });
      ...
      if (data.pattern) {
        linesEl.createDiv({ cls: 'claudian-plus-tool-line', text: `Pattern: ${data.pattern}` });
      }
      return true;
    ...
    default: {
      ...
      for (const query of alternateQueries.slice(0, 4)) {
        linesEl.createDiv({ cls: 'claudian-plus-tool-line', text: `Alt query: ${query}` });
      }
      if (alternateQueries.length > 4) {
        linesEl.createDiv({
          cls: 'claudian-plus-tool-truncated',
...
function renderWebSearchExpanded(
  ...
  if (parsed && parsed.links.length > 0) {
    const linksEl = container.createDiv({ cls: 'claudian-plus-tool-lines' });
    for (const link of parsed.links) {
      appendToolLink(linksEl, link.title, link.url);
    }

    if (parsed.summary) {
      const summaryEl = container.createDiv({ cls: 'claudian-plus-tool-web-summary' });
  ...
  container.createDiv({ cls: 'claudian-plus-tool-empty', text: 'No result' });
}

function renderFileSearchExpanded(container: HTMLElement, result: string): void {
  const lines = result.split(/\r?\n/).filter(line => line.trim());
  if (lines.length === 0) {
    container.createDiv({ cls: 'claudian-plus-tool-empty', text: 'No matches found' });
    return;
  }
...
function renderLinesExpanded(
  ...
  for (const line of displayLines) {
    ...
    const lineEl = linesEl.createDiv({ cls: 'claudian-plus-tool-line' });
...
function renderToolSearchExpanded(container: HTMLElement, result: string): void {
  ...
  for (const name of toolNames) {
    const lineEl = container.createDiv({ cls: 'claudian-plus-tool-search-item' });
    const iconEl = lineEl.createSpan({ cls: 'claudian-plus-tool-search-icon' });
...
function renderWebFetchExpanded(container: HTMLElement, result: string): void {
  const maxChars = 500;
  const linesEl = container.createDiv({ cls: 'claudian-plus-tool-lines' });
  const lineEl = linesEl.createDiv({ cls: 'claudian-plus-tool-line claudian-plus-tool-line-wrap' });
...
function renderApplyPatchExpanded(
  ...
  if (changes.length > 0) {
    ...
    for (const change of changes as unknown[]) {
      ...
      linesEl.createDiv({ cls: 'claudian-plus-tool-line', text: pathText });
    }
    return;
  }
...
function renderApplyPatchDiffSections(
  ...
  for (const fileDiff of fileDiffs) {
    const sectionEl = container.createDiv({ cls: 'claudian-plus-tool-patch-section' });

    if (fileDiff.operation === 'delete' && fileDiff.diffLines.length === 0) {
      sectionEl.createDiv({ cls: 'claudian-plus-tool-empty', text: 'File deleted' });
      continue;
    }

    if (fileDiff.diffLines.length === 0) {
      sectionEl.createDiv({ cls: 'claudian-plus-tool-empty', text: 'No textual diff available' });
      continue;
    }

    const diffRow = sectionEl.createDiv({ cls: 'claudian-plus-write-edit-diff-row' });
    const diffEl = diffRow.createDiv({ cls: 'claudian-plus-write-edit-diff' });
...
function resetStatusElement(statusEl: HTMLElement, statusClass: string, ariaLabel: string): void {
  statusEl.className = 'claudian-plus-tool-status';
  statusEl.empty();
...
function setApplyPatchHeaderRight(statusEl: HTMLElement, toolCall: ToolCallInfo): void {
  ...
  statusEl.className = 'claudian-plus-tool-status claudian-plus-write-edit-stats';
  statusEl.empty();
...
export function renderTodoWriteResult(
  ...
  container.empty();
  container.addClass('claudian-plus-todo-panel-content');
  container.addClass('claudian-plus-todo-list-container');

  const todos = input.todos as TodoItem[] | undefined;
  if (!todos || !Array.isArray(todos)) {
    const item = container.createSpan({ cls: 'claudian-plus-tool-result-item' });
...
function createToolElementStructure(
  ...
  const toolEl = parentEl.createDiv({ cls: 'claudian-plus-tool-call' });
  if (toolCall.name === TOOL_BASH) {
    toolEl.addClass('claudian-plus-tool-call-bash');
  }

  const header = toolEl.createDiv({ cls: 'claudian-plus-tool-header' });
  header.setAttribute('tabindex', '0');
  header.setAttribute('role', 'button');

  const iconEl = header.createSpan({ cls: 'claudian-plus-tool-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setToolIcon(iconEl, toolCall.name);

  const nameEl = header.createSpan({ cls: 'claudian-plus-tool-name' });
  nameEl.setText(getToolName(toolCall.name, toolCall.input));

  const summaryEl = header.createSpan({ cls: 'claudian-plus-tool-summary' });
  ...
  const statusEl = header.createSpan({ cls: 'claudian-plus-tool-status' });

  const content = toolEl.createDiv({ cls: 'claudian-plus-tool-content' });
...
function renderAskUserQuestionResult(container: HTMLElement, toolCall: ToolCallInfo): boolean {
  ...
  const reviewEl = container.createDiv({ cls: 'claudian-plus-ask-review' });
  for (let i = 0; i < questions.length; i++) {
    const q = questions[i];
    const answer = formatAnswer(
      (q.id ? answers[q.id] : undefined) ?? answers[q.question]
    );
    const pairEl = reviewEl.createDiv({ cls: 'claudian-plus-ask-review-pair' });
    pairEl.createDiv({ text: `${i + 1}.`, cls: 'claudian-plus-ask-review-num' });
    const bodyEl = pairEl.createDiv({ cls: 'claudian-plus-ask-review-body' });
    bodyEl.createDiv({ text: q.question, cls: 'claudian-plus-ask-review-q-text' });
    bodyEl.createDiv({
      text: answer || 'Not answered',
      cls: answer ? 'claudian-plus-ask-review-a-text' : 'claudian-plus-ask-review-empty',
    });
  }
...
function renderAskUserQuestionFallback(container: HTMLElement, toolCall: ToolCallInfo, initialText?: string): void {
  ...
  if (initialText || toolCall.result) {
    container.createDiv({
      cls: 'claudian-plus-ask-review-prompt',
  ...
  for (let questionIndex = 0; questionIndex < questions.length; questionIndex++) {
    ...
    const reviewEl = container.createDiv({ cls: 'claudian-plus-ask-review' });
    const pairEl = reviewEl.createDiv({ cls: 'claudian-plus-ask-review-pair' });
    pairEl.createDiv({ text: `${questionIndex + 1}.`, cls: 'claudian-plus-ask-review-num' });
    const bodyEl = pairEl.createDiv({ cls: 'claudian-plus-ask-review-body' });
    bodyEl.createDiv({ text: question.question, cls: 'claudian-plus-ask-review-q-text' });

    if (!Array.isArray(question.options) || question.options.length === 0) {
      bodyEl.createDiv({ cls: 'claudian-plus-ask-review-empty', text: 'No options recorded' });
      continue;
    }

    const listEl = bodyEl.createDiv({ cls: 'claudian-plus-ask-list' });
...
function renderAskUserQuestionOption(
  ...
  const itemEl = parentEl.createDiv({ cls: 'claudian-plus-ask-item is-disabled' });

  if (isMultiSelect) {
    itemEl.createDiv({ cls: 'claudian-plus-ask-check', text: '[ ] ' });
  } else {
    itemEl.createDiv({ cls: 'claudian-plus-ask-item-num', text: `${optionIndex + 1}. ` });
  }

  const contentEl = itemEl.createDiv({ cls: 'claudian-plus-ask-item-content' });
  const labelRowEl = contentEl.createDiv({ cls: 'claudian-plus-ask-label-row' });
  labelRowEl.createDiv({ cls: 'claudian-plus-ask-item-label', text: option.label });

  if (option.description) {
    contentEl.createDiv({ cls: 'claudian-plus-ask-item-desc', text: option.description });
  }
}

function contentFallback(container: HTMLElement, text: string): void {
  const resultRow = container.createDiv({ cls: 'claudian-plus-tool-result-row' });
  const resultText = resultRow.createSpan({ cls: 'claudian-plus-tool-result-text' });
...
function renderBashContent(
  ...
  if (command) {
    const cmdEl = container.createDiv({ cls: 'claudian-plus-tool-bash-command' });
...
function createCurrentTaskPreview(
  ...
  const currentTaskEl = header.createSpan({ cls: 'claudian-plus-tool-current' });
...
function createTodoToggleHandler(
  ...
  return (expanded: boolean) => {
    ...
    if (currentTaskEl) {
      currentTaskEl.toggleClass('claudian-plus-hidden', expanded);
    }
    if (statusEl) {
      statusEl.toggleClass('claudian-plus-hidden', expanded);
    }
  };
}
...
function renderToolContent(
  ...
  if (toolCall.name === TOOL_TODO_WRITE) {
    content.addClass('claudian-plus-tool-content-todo');
    renderTodoWriteResult(content, toolCall.input);
  } else if (toolCall.name === TOOL_ASK_USER_QUESTION) {
    content.addClass('claudian-plus-tool-content-ask');
    if (initialText) {
...
export function updateToolCallResult(
  ...
  if (toolCall.name === TOOL_TODO_WRITE) {
    const statusEl = toolEl.querySelector('.claudian-plus-tool-status') as HTMLElement;
    if (statusEl) {
      setTodoWriteStatus(statusEl, toolCall.input);
    }
    const content = toolEl.querySelector('.claudian-plus-tool-content') as HTMLElement;
    if (content) {
      renderTodoWriteResult(content, toolCall.input);
    }
    const nameEl = toolEl.querySelector('.claudian-plus-tool-name') as HTMLElement;
    if (nameEl) {
      nameEl.setText(getToolName(toolCall.name, toolCall.input));
    }
    const currentTaskEl = toolEl.querySelector('.claudian-plus-tool-current') as HTMLElement;
    if (currentTaskEl) {
