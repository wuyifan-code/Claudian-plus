...
export class ClaudianPlusView extends ItemView {
  ...
  // Initial tab restoration is deliberately scheduled after the first paint.
  // Keep a promise barrier so commands and clicks that arrive during that
  // window still observe a fully restored tab manager.
  private initialTabRestoreFrame: ScheduledAnimationFrame | null = null;
  private initialTabRestorePromise: Promise<void> | null = null;
  private initialTabRestoreResolve: (() => void) | null = null;
  ...
  constructor(leaf: WorkspaceLeaf, plugin: FeatureHost) {
    ...
    // overwritten by prototype patching. Hover Editor patches ClaudianPlusView.prototype.load
  ...
  getViewType(): string {
    return VIEW_TYPE_CLAUDIAN_PLUS;
  }
  ...
  private async onOpenImpl() {
    ...
    this.viewContainerEl.addClass('claudian-plus-container');

    const header = this.viewContainerEl.createDiv({ cls: 'claudian-plus-header' });
    ...
    // Attach the navigation row before restoring tabs so the first restored
    // shell can be mounted without extending the view-open critical path.
    this.attachNavRowContentToInputFooter();
    this.updateInputLocation();
    this.updateTabBarVisibility();
    this.scheduleInitialTabRestore();
    this.syncProviderBrandColor();
  }

  async onClose() {
    this.isClosing = true;
    if (this.initialTabRestoreFrame) {
      cancelScheduledAnimationFrame(this.initialTabRestoreFrame);
      this.initialTabRestoreFrame = null;
      this.initialTabRestoreResolve?.();
      this.initialTabRestoreResolve = null;
      this.initialTabRestorePromise = null;
    }
    const pendingInitialRestore = this.initialTabRestorePromise;
    if (pendingInitialRestore) {
      await pendingInitialRestore.catch(() => undefined);
    }
  ...
  private buildNavRowContent(): HTMLElement {
    const wrapper = this.containerEl.createDiv({ cls: 'claudian-plus-input-nav-content' });

    this.tabBarContainerEl = wrapper.createDiv({ cls: 'claudian-plus-tab-bar-container' });
    ...
    const navActionsEl = wrapper.createDiv({ cls: 'claudian-plus-input-nav-actions' });

    this.newTabButtonEl = navActionsEl.createDiv({ cls: 'claudian-plus-input-nav-btn claudian-plus-new-tab-btn' });
    ...
    const newBtn = navActionsEl.createDiv({ cls: 'claudian-plus-input-nav-btn' });
    ...
    // History dropdown
    const historyContainer = navActionsEl.createDiv({ cls: 'claudian-plus-history-container' });
    const historyBtn = historyContainer.createDiv({ cls: 'claudian-plus-input-nav-btn' });
    setIcon(historyBtn, 'history');
    historyBtn.setAttribute('aria-label', 'Chat history');

    this.historyDropdown = historyContainer.createDiv({ cls: 'claudian-plus-history-menu' });
  ...
  private buildInputFooter(): void {
    ...
    this.inputFooterEl = this.viewContainerEl.createDiv({ cls: 'claudian-plus-input-footer' });
    this.inputNavRowHostEl = this.inputFooterEl.createDiv({
      cls: 'claudian-plus-input-nav-row claudian-plus-view-input-nav-row',
    });
    this.activeInputSlotEl = this.inputFooterEl.createDiv({ cls: 'claudian-plus-active-input-slot' });
  }
  ...
  private handleTabClick(tabId: TabId): void {
    void this.whenReady()
      .then(() => this.tabManager?.switchToTab(tabId))
      .catch(() => new Notice('Failed to switch tab'));
  }
  ...
  private updateTabBarVisibility(): void {
    ...
    this.tabBarContainerEl.toggleClass('claudian-plus-hidden', !showTabBar);
  ...
  private updateNewTabButtonVisibility(): void {
    ...
    this.newTabButtonEl.toggleClass('claudian-plus-hidden', !canCreateTab);
    if (canCreateTab) {
  ...
  /** Waits until the initial restored tab shells are available. */
  async whenReady(): Promise<void> {
    await this.initialTabRestorePromise;
  }

  /** Schedule the initial restore after the view has had a chance to paint. */
  private scheduleInitialTabRestore(): void {
    if (this.initialTabRestorePromise || this.initialTabRestoreFrame || this.isClosing) return;

    let resolveRestore!: () => void;
    this.initialTabRestorePromise = new Promise<void>((resolve) => {
      resolveRestore = resolve;
    });
    this.initialTabRestoreResolve = resolveRestore;
    const restorePromise = this.initialTabRestorePromise;
    const restoreFrame = scheduleAnimationFrame(() => {
      this.initialTabRestoreFrame = null;
      if (this.isClosing) {
        resolveRestore();
        this.initialTabRestoreResolve = null;
      ...
      void this.restoreOrCreateTabs()
        .catch((error: unknown) => {
          new Notice(`Failed to restore Claudian Plus tabs: ${error instanceof Error ? error.message : String(error)}`);
        })
        .finally(() => {
          if (this.initialTabRestoreResolve === resolveRestore) {
            this.initialTabRestoreResolve = null;
          }
          if (this.initialTabRestorePromise) {
            this.initialTabRestorePromise = null;
          }
          resolveRestore();
        });
    }, this.containerEl.ownerDocument.defaultView ?? null);
    // A server-side/test renderer may execute the fallback callback
    // synchronously. Do not leave a stale frame handle in that case.
    if (this.initialTabRestorePromise === restorePromise) {
      this.initialTabRestoreFrame = restoreFrame;
    }
  }
   ...
   * Used by the file explorer "Add to Claudian Plus" menu action.
   */
