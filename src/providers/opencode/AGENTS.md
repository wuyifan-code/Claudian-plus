...
- History hydration reads OpenCode's native SQLite database. Never mutate OpenCode native history from Claudian Plus.
...
- `prepareOpencodeLaunchArtifacts()` writes managed config and system prompt files under `.claudian-plus/opencode/`.
...
- OpenCode runtime commands are not editable or deletable from Claudian Plus.
