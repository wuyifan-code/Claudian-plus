...
class ForkTargetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('claudian-plus-fork-target-modal');

    const list = this.contentEl.createDiv({ cls: 'claudian-plus-fork-target-list' });
  ...
  private createOption(container: HTMLElement, target: ForkTarget, label: string): void {
    const item = container.createDiv({ cls: 'claudian-plus-fork-target-option', text: label });
