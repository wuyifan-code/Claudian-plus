 ...
 */
export const XML_CONTEXT_PATTERN = /\n\n<(?:linked_note|current_note|editor_selection|editor_cursor|context_files|canvas_selection|browser_selection|vault_context|ambient_context)[\s>]/;
...
export function extractUserQuery(prompt: string): string {
  ...
  return prompt
    ...
    .replace(/<ambient_context>[\s\S]*?<\/ambient_context>\s*/g, '')
    .trim();
}
