...
describe('ClaudianPlusSettingsStorage', () => {
  let storage: ClaudianPlusSettingsStorage;
  ...
  beforeEach(() => {
    ...
    storage = new ClaudianPlusSettingsStorage(mockAdapter);
  });
  ...
  describe('load', () => {
    ...
    it('loads legacy settings and migrates them to .claudian-plus', async () => {
      ...
      mockAdapter.read.mockImplementation(async (path: string) => {
        if (path === LEGACY_CLAUDIAN_PLUS_SETTINGS_PATH) {
      ...
      expect(mockAdapter.delete).toHaveBeenCalledWith(LEGACY_CLAUDIAN_PLUS_SETTINGS_PATH);
    });
  ...
  describe('exists', () => {
    it('should return true when the new file exists', async () => {
      mockAdapter.exists.mockImplementation(async (path: string) => (
        path === CLAUDIAN_PLUS_SETTINGS_PATH
      ));

      const result = await storage.exists();

      expect(result).toBe(true);
      expect(mockAdapter.exists).toHaveBeenCalledWith(CLAUDIAN_PLUS_SETTINGS_PATH);
    });
    ...
    it('should return true when only the legacy file exists', async () => {
      ...
      expect(result).toBe(true);
      expect(mockAdapter.exists).toHaveBeenCalledWith(CLAUDIAN_PLUS_SETTINGS_PATH);
      expect(mockAdapter.exists).toHaveBeenCalledWith(LEGACY_CLAUDIAN_PLUS_SETTINGS_PATH);
    });
